...
stepcount = 5
params = ModelParams(sidelength=20, density=0.8, m_barabasi=2, social_factor=0.95, connections_per_step=5, fermi_alpha=1, fermi_b=3, opinion_max_diff=2, total_steps = 5, happiness_threshold = 0.6)
...
# index_labels = list(range(stepcount))*reps
# all_data.set_axis(index_labels, axis='index')
# print(all_data)

# by_row_index = model_df.groupby(model_df.index)
...
#%% 
plt.figure(dpi=300)
plt.plot(range(stepcount+1), average_mod, label = "Mean progression of Modularity", linewidth = 0.9)
plt.fill_between(range(stepcount+1), average_mod-mod_stds, average_mod+mod_stds, color='skyblue', alpha=0.5)
...
plt.figure(dpi=300)
plt.plot(range(stepcount+1), average_entropy, label = "Mean progression of Modularity", linewidth = 0.9)
plt.fill_between(range(stepcount+1), average_entropy-ent_stds, average_entropy+ent_stds, color='skyblue', alpha=0.5)
plt.xlabel("Time steps")
